...
fn fill_nvidia_stats(stats: &mut GpuStats, pci_id: &str) {
    let output = Command::new("sudo")
        .args(&["-n", "nvidia-smi", "-i", pci_id, "--query-gpu=name,utilization.gpu,memory.used,memory.total,temperature.gpu", "--format=csv,noheader,nounits"])
        .output();
    ...
    if let Ok(o) = output {
        ...
        if let Ok(o) = Command::new("sudo").args(&["-n", "nvidia-smi", "--query-gpu=name,utilization.gpu,memory.used,memory.total,temperature.gpu", "--format=csv,noheader,nounits"]).output() {
            if o.status.success() {
                let stdout = String::from_utf8_lossy(&o.stdout);
...
fn fill_intel_stats(stats: &mut GpuStats, card_name: &str) {
    ...
    let has_intel_top = Command::new("which").arg("intel_gpu_top").output().map(|o| o.status.success()).unwrap_or(false);
    if has_intel_top {
        let output = Command::new("sudo")
                ...
                if let Ok(v) = serde_json::from_str::<serde_json::Value>(&stdout[json_start..]) {
                    if let Some(engines) = v.get("engines").and_then(|e| e.as_object()) {
                        ...
                        for (name, engine) in engines {
                            if name.contains("Render") || name.contains("3D") || name.contains("Video") {
                                let busy = if let Some(b) = engine.get("busy") {
                                    if let Some(val) = b.as_f64() { Some(val) }
                                    else if let Some(val) = b.get("value").and_then(|v| v.as_f64()) { Some(val) }
                                    else { None }
                                } else { None };

                                if let Some(val) = busy {
                                    if val > max_busy { max_busy = val; }
                                }
                            }
                        }
    ...
    if stats.load == 0.0 {
        let act_path = format!("/sys/class/drm/{}/device/drm/{}/gt_act_freq_mhz", card_name, card_name);
        let max_path = format!("/sys/class/drm/{}/device/drm/{}/gt_max_freq_mhz", card_name, card_name);
        
        if let (Ok(act_s), Ok(max_s)) = (std::fs::read_to_string(&act_path), std::fs::read_to_string(&max_path)) {
    ...
    if scan_hwmon_for_temp(format!("{}/hwmon", device_path), stats) { return; }
    
    // 2. Try nested hwmon
    if scan_hwmon_for_temp(format!("{}/device/hwmon", device_path), stats) { return; }
        ...
        for entry in hwmon_dir.flatten() {
            ...
            if let Ok(name) = std::fs::read_to_string(path.join("name")) {
                ...
                if name == "i915" || name == "xe" || name.contains("intel_gpu") {
                    if scan_hwmon_dir(&path, stats) { return; }
                }
            }
        }
    }
...
fn scan_hwmon_for_temp(path: String, stats: &mut GpuStats) -> bool {
    if let Ok(hwmon_dir) = std::fs::read_dir(path) {
        for entry in hwmon_dir.flatten() {
            if scan_hwmon_dir(&entry.path(), stats) { return true; }
        }
    }
    false
}
            ...
            if file_name.starts_with("temp") && file_name.ends_with("_input") {
                if let Ok(temp_str) = std::fs::read_to_string(file.path()) {
                    ...
                    if t > 0.0 && t < 150.0 {
                        ...
                        let label = std::fs::read_to_string(label_path).unwrap_or_default().to_lowercase();
                        
                        if label.contains("gpu") || label.contains("composite") {
                            stats.temp = t;
                            return true;
                        }
                        if t > best_temp { best_temp = t; }
                    }
                }
            }
        }
...
pub fn get_gpu_stats() -> Vec<GpuStats> {
    ...
    if let Ok(entries) = std::fs::read_dir("/sys/class/drm") {
        let mut card_entries: Vec<_> = entries.flatten()
    ...
    for pci_id in pci_gpus {
        if !detected_pci_ids.contains(&pci_id) {
            ...
            let vendor_id = std::fs::read_to_string(format!("/sys/bus/pci/devices/{}/vendor", pci_id)).unwrap_or_default().trim().to_string();
    ...
    for (i, gpu) in gpus.iter_mut().enumerate() {
        ...
        log::trace!("GPU {}: {} (PCI: {}, Temp: {}°C)", i, gpu.name, gpu.pci_id, gpu.temp);
    }
...
fn get_interface_speed(iface: &str) -> u64 {
    if iface.is_empty() { return 125_000_000; }
...
fn count_services(state: &str) -> u32 {
    let output = Command::new("sudo")
        .args(&["-n", "systemctl", "list-units", "--type=service", &format!("--state={}", state), "--no-legend", "--no-pager"])
        .output();
    match output {
        Ok(out) => {
...
fn count_containers() -> u32 {
    let output = Command::new("sudo").args(&["-n", "docker", "ps", "-q"]).output();
    match output {
        Ok(out) => {
            let stdout = String::from_utf8_lossy(&out.stdout);
            stdout.lines().count() as u32
...
pub async fn get_system_stats(data: web::Data<AppState>) -> impl Responder {
    ...
    let (active_services, failed_services, active_containers, upgradable_packages, network_interface, network_max_speed, gpus) = actix_web::web::block(move || {
    ...
    }).await.unwrap_or((0, 0, 0, 0, String::new(), 0, Vec::new()));
